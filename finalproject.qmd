...
-   release_year: year of song released (1950-2018)
...
beat_strength, dyn_range_mean, bounciness, organism - Note: We decided to leave in danceability and mechanism because we felt that these variables are easily understandable from a musical perspective and may be important to the model.

The new regression model and corresponding correlation plot can be shown below:

```{r newcorplotvars, message = F, warning = F, echo = FALSE, eval = FALSE}
...
A second , but they are not as extreme and prevalent as prior models. Therefore, we feel that it is appropriate to proceed with the revised model.
...
```{r roc-curve, message = F, warning = F, echo = FALSE}
glm_aug |>
